ruleset driver_gossip_protocol {
  meta {
    logging on
    shares __testing, process_status, heartbeat_delay, random_message_type,
      ...
      get_registry, choose_peer, get_location
  ...
  global {
    __testing = { "queries": [
                ...
                "events": [
                  ...
                  {"domain": "gossip", "type": "generate_message", "attrs": 
                    ["Message"]},
                  {"domain": "gossip", "type": "registration_referral", "attrs": 
                    ["registry_Rx"]},
                  {"domain": "test", "type": "test_generate_contract"}]}
                  
                  /*
                  {"domain": "test", "type": "test_rumor_sensor", "attrs": 
                    ["MessageID", "SensorID", "Temperature", "Timestamp", "peer_Tx"]},
                  {"domain": "test", "type": "test_rumor_remote_sensor", "attrs": 
                    ["MessageID", "SensorID", "Temperature", "Timestamp", "peer_wellKnown_Tx"]},
                  {"domain": "test", "type": "test_generate_sensor", "attrs": 
    ...
    get_location = function() {
      location = ent:location.defaultsTo(
          {"latitude": 40.2968979 + random:number(lower = 0, upper = 1), 
    ...
    heartbeat_delay = function() {
      delay = ent:heartbeat_delay.defaultsTo({"measurement": "seconds", "magnitude": 5})
      delay
    }
  ...
  rule registry_referred {
    select when gossip registration_referral
    pre {
      registry_Rx = get_registry()
      node_Rx = wrangler:myself().get("eci")
      attrs = {"driver_Rx": node_Rx}
      location = get_location()
    }
    event:send({"eci": registry_Rx, "eid": "driver_registration", "domain": "registry", 
          "type": "driver_registration", "attrs": attrs})
    fired {
      ent:registry_Rx := registry_Rx
  ...
  rule generate_message {
    ...
    pre {
      ...
      sequence_number = peer_messages().get([node_Tx, DriverID])
        .defaultsTo({}).length()
      MessageID = DriverID + ":" + sequence_number.as("String")
      Message = event:attrs.get("Message")
    ...
    fired {
      ent:peer_message_maps := peer_messages().put([node_Tx, DriverID, sequence_number], Message)
      ent:peer_sequence_number_maps := peer_sequence_numbers().put([node_Tx, DriverID], sequence_number)
    }
  }
                    
  rule contract_generate_test {
    select when test test_generate_contract
    pre {
      standard_latitude = 40.2968979
      random_latitude = (standard_latitude + random:number(lower = 0, upper = 1))
        .klog("Generated Contract Latitude: ")
        
      standard_longitude= -111.69464749999997
      random_longitude = (standard_longitude + random:number(lower = 0, upper = 1))
        .klog("Generated Contract Longitude: ")
      
      location = {"latitude": random_latitude, "longitude": random_longitude}
        .klog("Generated Contract Location: ")
        
      orderID = random:uuid()
        .klog("Generated Contract UUID: ")
      
      Message = {"location": location, "orderID": orderID}
        .klog("Generated Contract: ")
    }
    noop()
    fired {
  ...
  /*
  rule sensor_generate_test {
    select when test test_generate_sensor
    pre {
  ...
  */
  ...
  /*
  rule sensor_remote_rumor_test {
    select when test test_rumor_remote_sensor
    pre {
  ...
  */
  ...
  /*
  rule sensor_direct_rumor_test {
    select when test test_rumor_sensor
    pre {
      MessageID = event:attrs.get("MessageID")
      MessageID_components = MessageID.split(re#:#)
      OriginID = MessageID_components[0]
  ...
  rule set_delay {
    ...
    pre {
      ...
      raw_delay_magnitude = event:attrs.get("magnitude") == "" || event:attrs.get("magnitude").isnull() 
        => 5 | event:attrs.get("magnitude").as("Number").defaultsTo(5)
